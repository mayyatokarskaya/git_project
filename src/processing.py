...
def parse_date(record: Dict[str, str]) -> datetime:
    ...
    try:
        ...
        try:
            # Если не удалось, пробуем парсить дату в формате "%Y-%m-%dT%H:%M:%SZ" (для CSV и XLSX)
...
def sort_by_date(records: List[Dict[str, str]], reverse: bool = False) -> List[Dict[str, str]]:
    ...
    return sorted(
        ...
        key=lambda record: (
            parse_date(record),  # Сортировка по дате
            0 if record["state"] == "EXECUTED" else 1,  # При одинаковых датах сортируем по состоянию (EXECUTED первым)
            float(record["operationAmount"]["amount"]),  # Сортировка по сумме, если нужно
        ),
        reverse=reverse,
    )
