...
# Новый функционал для работы с банковскими транзакциями count_operations_by_category.py, search_tranzaction.py, main.py

Этот проект предоставляет набор функций для чтения, обработки и анализа банковских транзакций из различных источников: JSON, CSV и Excel. Программа позволяет фильтровать транзакции по различным параметрам, сортировать их, а также подсчитывать количество операций по категориям.

## Основные функции

### 1. Чтение данных
- `read_json(file_path)`: Читает данные из JSON-файла и возвращает их в виде списка словарей.
- `read_csv(file_path)`: Читает данные из CSV-файла и возвращает их в виде списка словарей.
- `read_excel(file_path)`: Читает данные из Excel-файла и возвращает их в виде списка словарей.

### 2. Обработка данных
- `extract_categories(operations)`: Извлекает уникальные категории (описания) из списка операций.
- `count_operations_by_category(operations, categories)`: Подсчитывает количество банковских операций по каждой категории.
- `filter_transactions_by_description(transactions, search_string)`: Фильтрует транзакции по описанию с использованием регулярных выражений.
- `filter_transactions_by_status(transactions)`: Фильтрует транзакции по заданному статусу (EXECUTED, CANCELED, PENDING).
- `sort_transactions(transactions)`: Сортирует транзакции по дате в заданном порядке.

### 3. Основная логика
- `process_data()`: Основная функция для чтения данных из файлов, извлечения категорий и подсчета операций.
- `main()`: Основная функция программы, которая интерактивно взаимодействует с пользователем, позволяя выбирать источник данных, фильтровать и сортировать транзакции.
